...
def test_simple_program() -> None:
    ...
    assert len(counts) == 1
    assert counts['0'] == 100
    np.testing.assert_allclose(state, [1.0, 0.0], atol=1e-8)
    ...
    assert len(counts) == 1
    assert counts['1'] == 100
    np.testing.assert_allclose(state, [0.0, 1.0], atol=1e-8)

    # Adding an h gate should bring the circuit into superposition.
    qc.h(0)
    counts, state = qc.measure(num_shots=100)
    assert len(counts) == 2
    np.testing.assert_allclose(state, [1 / np.sqrt(2), 1 / np.sqrt(2)], atol=1e-8)
    ...
    np.testing.assert_allclose(state, [1 / np.sqrt(2), 0., 0., 1 / np.sqrt(2)], atol=1e-8)
